...
function pkmmenu(id)
    ...
    return
    {
      ...
      {
        title = "{ffffff}» Âîïðîñû íà ñîáåñåäîâàíèå",
        onclick = function()
        pID = tonumber(args)
      ...
      {
        title = "{ffffff}» Ñîáåñåäîâàíèå êîíåö",
        onclick = function()
        pID = tonumber(args)
...
function sobes(id)
    return
    {
      ...
      {
        title = '{ffffff}» Ïðèâåòñòâèå è ïàñïîðò.',
        onclick = function()
		local _, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
        local myname = sampGetPlayerNickname(myid)
        sampSendChat("Çäðàâñòâóéòå. ß ñîòðóäíèê àâòîøêîëû "..myname:gsub('_', ' ')..", ïîêàæèòå êîìïëåêò âàøèõ äîêóìåíòîâ, à èìåííî ìåä êàðòó è ïàñïîðò.")
        wait(1500)
        sampSendChat("/b /showpass "..myid..", /me ïîêàçàë(à) ìåä.êàðòó") 
		end
      }
    }
end
...
function vopsobes(args)
    ...
    {
      {
        title = '{5b83c2}« Âîïðîñû äëÿ ñîáåñåäîâàíèÿ »',
        onclick = function()
        end
      },
      {
        title = '{ffffff}» Ïðîáëåìû ñ ãîëîâîé',
        onclick = function()
          sampSendChat("×òî ó ìåíÿ íàä ãîëîâîé?")
		end
      },
      {
        title = '{ffffff}» Òåðìèíû',
        onclick = function()
        sampSendChat("/b DM, MG, SK, TK, ìíå â ñìñ. /showpass "..myid.."")
        end
      }
    }
end
...
function sobesconec(id)
    return
    {
      ...
      {
        title = '{ffffff}» Ïðèíÿòèå',
        onclick = function()
		local _, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
        local myname = sampGetPlayerNickname(myid)
        sampSendChat("Ïîçäðàâëÿþ, âû ïðèíÿòû, ïðîéäèòå â êîìíàòó òåîðèè, ñåé÷àñ âàì ñîòðóäíèê ïðî÷èòàåò ëåêöèþ")
		end
      }
    }
end
