...
[1] Общее положение Автошколы: писал(а):
...
[2] Обязанности сотрудников Автошколы: писал(а):
...
? 2.9.5 Сотрудник Отдела Контроля обязан носить Бейджик №16.
? 2.9.6 Сотрудник Отдела Стажировки обязан носить Бейджик №6.
? 2.9.7 Ответственный за работу в Филиале Бейджик №26
? 2.9.8 Сотрудник который приходится Экзаменатором по заявке обязан носить Бейджик № 10.
...
[3] Рабочий день в Автошколе: писал(а):

? 3.1 В будни, рабочий день длится с 08:00 до 21:00. В выходные дни рабочий день длится с 9:00 до 20:00.
? 3.2 Время для перерыва (обеда) с 13:00 до 14:00.
? 3.3 В рабочее время запрещено носить любые аксессуары. (Исключение: береты, шляпы, очки, часы, усы, чемоданы и рюкзаки)
...
[4] Сотрудникам Автошколы запрещается: писал(а):
...
? 4.12 Спать более десяти минут (600 секунд) каждый час во время рабочего дня. Исключения: старший состав, работа в ночную смену, работа с оф. порталом (отчет) или разрешение Управляющего. (
...
? 4.23 Запрещено писать в департамент во время ЧС- Чрезвычайной ситуации.


[5] Иерархия и руководство отделов автошколы: писал(а):
...
[6] Наказания за нарушение устава: писал(а):
...
Изображение · Профессиональная этика писал(а):
...
Изображение · Примечания: писал(а):
...
{
 {
     ...
     sampSendChat("/me полил цветы с ведра")
     wait(cfg.commands.zaderjka * 1000)
 ...
 },
 {
     ...
     onclick = function()
     sampSendChat('/me наклонившись нажал кнопку, от чего компьютер включился')
     wait(cfg.commands.zaderjka * 1000)
     sampSendChat('/do Компьютер издал звук, чем дал понять что он включился')
     wait(cfg.commands.zaderjka * 1000)
     sampSendChat('/me взяв в руки мышку открыл браузер, после чего скачал антивирус')
     wait(cfg.commands.zaderjka * 1000)
     sampSendChat('/do Антивирус скачался')
     wait(cfg.commands.zaderjka * 1000)
     sampSendChat('/me открыл антивирус, после чего запустил проверку от вирусов')
     wait(cfg.commands.zaderjka * 1000)
 ...
 },
 {
     ...
     wait(cfg.commands.zaderjka * 1000)
     sampSendChat('/me поставил ведро с водой на землю, после чего взяв тряпку намочил ее и выжал')
     wait(cfg.commands.zaderjka * 1000)
     sampSendChat('/me протер тряпкой фары')
     wait(cfg.commands.zaderjka * 1000)
     sampSendChat('/me протер тряпкой корпус машины')
     wait(cfg.commands.zaderjka * 1000)
     sampSendChat('/me опять намочил тряпку, после чего выжал ее')
     wait(cfg.commands.zaderjka * 1000)
     sampSendChat('/me протер стекло машины')
     wait(cfg.commands.zaderjka * 1000)
...
function pkmmenu(id)
    ...
    return
    {
      ...
      },
      {
        ...
        onclick = function()
        pID = tonumber(args)
        submenus_show(sobes(id), "{9966cc}Inst Tools {ffffff}| {"..color.."}"..sampGetPlayerNickname(id).."["..id.."] ")
        end
      },
      {
        ...
        onclick = function()
        pID = tonumber(args)
        submenus_show(rascenki(id), "{9966cc}Inst Tools {ffffff}| {"..color.."}"..sampGetPlayerNickname(id).."["..id.."] ")
        end
      },
	  {
...
function sobes(id)
    return
    {
      ...
      {
        title = '{ffffff}» Бланк с ответами, ДМ, МГ и тд. в sms',
        onclick = function()
        local _, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
        local myname = sampGetPlayerNickname(myid)
